...
TASK_NAME = "Isaac-Flat-G129-SONIC-Official"
...
def main() -> int:
    ...
    log_path = output_dir / f"sonic_official_baseline_{started_utc.strftime('%Y%m%dT%H%M%SZ')}.json"
    ...
    try:
        ...
        if len(names) != 29 or set(names) != set(G1_MOTOR_JOINTS):
            raise RuntimeError(
                f"Official baseline joint contract mismatch: count={len(names)}, names={names}"
            )
        print(f"[baseline] verified 29-DOF joint contract; native order={names}")
